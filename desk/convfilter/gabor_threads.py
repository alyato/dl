...
if __name__ == '__main__':
    ...
    try:
       ...
       # img_fn = '../../Downloads/face.jpg'
        im=Image.open('/home/hey/Downloads/face.jpg')
        pim=im.convert('L')
        imarr=np.array(pim)
        #print(imarr.shape)
        #print(pim)
    ...
    if imarr is None:
        print 'Failed to load image file:', im
        sys.exit(1)

    filters = build_filters()

    with Timer('running single-threaded'):
        res1 = process(imarr, filters)
    with Timer('running multi-threaded'):
        res2 = process_threaded(imarr, filters)

    print 'res1 == res2: ', (res1 == res2).all()
    cv2.imshow('img', imarr)
